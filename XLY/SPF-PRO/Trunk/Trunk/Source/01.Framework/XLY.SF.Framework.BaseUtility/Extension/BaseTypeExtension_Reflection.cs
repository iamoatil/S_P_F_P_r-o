...
/*************************************************
 * 创建人：Bob
 * 创建时间：2017/3/15 14:44:40
 * 类功能说明：整个类没有搞懂他要实现的效果，所以全部复制
 *
 *************************************************/
...
namespace XLY.SF.Framework.BaseUtility
{
    public static partial class BaseTypeExtension
    {
        ...
        public static void RemoveEvent<T>(this T obj, string eventName = null)
        {
            ...
            {
                fields.AddRange(obj.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public | BindingFlags.Static));
            }
            ...
            foreach (var f in fields)
            {
                ...
                if (fv != null && fv is Delegate)
                {
                    Delegate objectDelegate = (Delegate)fv;
                    Delegate[] invokeList = objectDelegate.GetInvocationList();
                    foreach (Delegate del in invokeList)
                    {
                        try
                        {
                            typeof(T).GetEvent(f.Name).RemoveEventHandler(obj, del);
                        }
                        catch { }
                    }
                }
        ...
        public static void RemoveData<T>(this T obj, string dataName = null)
        {
            List<FieldInfo> fields = new List<FieldInfo>();
            if (dataName != null)
            {
                FieldInfo field = obj.GetType().GetField(dataName, BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public | BindingFlags.Static);
                if (field != null)
                {
                    fields.Add(field);
                }
            ...
            foreach (var f in fields)
            {
                ...
                if (fv != null)
                {
                    if (fv is System.Collections.IList)
                    {
                        ((System.Collections.IList)fv).Clear();
                    }
                    if (fv.GetType().IsClass)
                    {
                        f.SetValue(obj, null);
                    }
                }
