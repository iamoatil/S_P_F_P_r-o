...
// Author:Songbing
// Created:2017-04-12 10:00:14
// Description:验证数据库表字段是否存在
// ***********************************************************************
// Last Modified By:
// Last Modified On:
// ***********************************************************************
...
namespace XLY.SF.Project.Plugin.Adapter.FeatureMathch
{
    ...
    /// 验证数据库表字段是否存在
    /// </summary>
    internal class FieldExistPluginFeatureRule:IFeatureMathchRule
    {
        ...
        public string TryMathch(string sourcePath)
        {
            ...
            if(arr.IsValid())
            {
                ...
                if(files.Any((f) => DbHelper.IsExistTableField(f, TableName, FiledName, Decryted)))
                {
                    return Success;
                }
                else
