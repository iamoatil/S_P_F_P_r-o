...
using MediaPlayer = XLY.XDD.Control.MediaPlayer;
...
namespace XLY.XDD.Control
{
    ...
    /// </summary>
    public class MediaElement : FrameworkElement, IFileViewer
    {
        ...
        //==========================================================================                
        /// <summary>
        ///   Gets or sets the source of the media loaded in the media element.播放的媒体
        /// </summary>
        public Uri Source
        {
            get
        ...
        private void OnCurrentVideoStreamChanged(VideoStream oldValue, VideoStream newValue)
        {
            if (MediaPlayer != null)
            {
                int index = -1;
                if (newValue != null)
                    index = VideoStreams.ToList().IndexOf(newValue);
        ...
        private void OnCurrentAudioStreamChanged(AudioStream oldValue, AudioStream newValue)
        {
            if (MediaPlayer != null)
            {
                int index = -1;
                if (newValue != null)
                    index = AudioStreams.ToList().IndexOf(newValue);
        ...
        private void OnCurrentSubtitleStreamChanged(SubtitleStream oldValue, SubtitleStream newValue)
        {
            if (MediaPlayer != null)
            {
                int index = -1;
                if (newValue != null)
                    index = SubtitleStreams.ToList().IndexOf(newValue);
        ...
        //==========================================================================
        private static readonly LibVLCLibrary m_Library = null;
        ...
        static MediaElement()
        {
            if (!(bool)(DesignerProperties.IsInDesignModeProperty.GetMetadata(typeof(DependencyObject)).DefaultValue))
            {
                m_Library = LibVLCLibrary.Load(AppDomain.CurrentDomain.BaseDirectory);
            }
