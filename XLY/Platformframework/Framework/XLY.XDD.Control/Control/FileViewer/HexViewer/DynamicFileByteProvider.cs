...
using XLY.XDD.Control.ReadToTxt;
...
namespace Be.Windows.Forms
{
    ...
    public sealed class DynamicFileByteProvider : IByteProvider, IDisposable
    {
        ...
        /// <param name="fileName">The name of the file from which bytes should be provided.</param>
        /// <param name="readOnly">True, opens the file in read-only mode.</param>
        public DynamicFileByteProvider(string fileName, bool readOnly, bool isGoToGetDefaultCharset)
        {
            _fileName = fileName;
            ...
            if (isGoToGetDefaultCharset)
            {
                try
                {
                    CharsetDetector detector = new CharsetDetector();
                    detector.Feed(_stream);
                    detector.DataEnd();
                   
                    if (detector.Charset.IsValid())
                    {
                        this._encoding = Encoding.GetEncoding(detector.Charset);
                    }
                    else
                    {
        ...
        /// The stream must supported seek operations.
        /// </remarks>
        public DynamicFileByteProvider(Stream stream,Encoding encoding)
        {
            if (stream == null)
                throw new ArgumentNullException("stream");
            if (!stream.CanSeek)
