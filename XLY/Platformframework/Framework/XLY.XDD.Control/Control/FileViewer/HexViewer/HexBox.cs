...
namespace Be.Windows.Forms
{
    ...
    public class HexBox : Control
    {
        ...
        public event Action<string> EncodingPain;

        public event Action<System.IO.Stream> EncodingPain_Stream;

        public event Action<string> EncodingBase64Pain;

        public event Action<System.IO.Stream> EncodingBase64Pain_Stream;

        public bool IsUseBase64;

        public bool IsUseEncoding;

        public void OnEncodingPain(string filepath)
        {
            if (this.EncodingPain != null)
            {
                this.EncodingPain(filepath);
            }
        }

        public void OnEncodingPain(System.IO.Stream stream)
        {
            if (this.EncodingPain_Stream != null)
            {
                this.EncodingPain_Stream(stream);
            }
        }

        public void OnEncodingBase64Pain(string filepath)
        {
            if (this.EncodingBase64Pain != null)
            {
                this.EncodingBase64Pain(filepath);
            }
        }

        public void OnEncodingBase64Pain(System.IO.Stream stream)
        {
            if (this.EncodingBase64Pain_Stream != null)
            {
                this.EncodingBase64Pain_Stream(stream);
            }
        }
        ...
        /// 更新垂直滚动条时触发
        /// </summary>
        public event EventHandler<UpdateVScrollEventArgs> OnUpdateVScroll;
        ...
        public HexBox()
        {
            ...
            Font = new Font("Courier New", (float)FontPerLine, FontStyle.Regular, GraphicsUnit.Point, ((byte)(0)));
        ...
        /// 初始化
        /// </summary>
        /// <param name="hexViewer"></param>
        public void Init(HexViewer hexViewer)
        {
            this._builtInContextMenu = new BuiltInContextMenu(this, hexViewer);
        }

        #endregion

        #region Scroll methods
        public void _vScrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            switch (e.Type)
            {
        ...
        void UpdateScrollSize()
        {
            ...
            if (VScrollBarVisible && _byteProvider != null && _byteProvider.Length > 0 && _iHexMaxHBytes != 0)
            {
                ...
                {
                    //UpdateVScrollEventArgs args = new UpdateVScrollEventArgs();
                    //args.Maximum = scrollmax;
                    //this.OnUpdateVScroll(this, args);
                    return;
                }
        ...
        public void UpdateVScroll()
        {
            ...
            if (this.OnUpdateVScroll != null)
            {
                args.Minimum = this._vScrollBar.Minimum;
                args.Maximum = this._vScrollBar.Maximum;
                args.Value = this._vScrollBar.Value;
                this.OnUpdateVScroll(this, args);
            }
        }
        ...
        /// 切换到ASCII码
        /// </summary>
        public void ChangeASCII()
        {
            this._encoding = Encoding.ASCII;
            if (!this.fileName.IsNullOrEmptyOrWhiteSpace())
                OnEncodingPain(fileName);
            if (this.stream != null)
                OnEncodingPain(stream);
        ...
        public void ChangeBig5()
        {
            this._encoding = Encoding.GetEncoding("big5");
            if (!this.fileName.IsNullOrEmptyOrWhiteSpace())
                OnEncodingPain(fileName);
            if (this.stream != null)
                OnEncodingPain(stream);
        ...
        public void ChangeBGb2312()
        {
            this._encoding = Encoding.GetEncoding("gb2312");
            if (!this.fileName.IsNullOrEmptyOrWhiteSpace())
                OnEncodingPain(fileName);
            if (this.stream != null)
                OnEncodingPain(stream);
        ...
        public void ChangeBase64()
        {
            if (!this.fileName.IsNullOrEmptyOrWhiteSpace())
                OnEncodingBase64Pain(fileName);
            if (this.stream != null)
                OnEncodingBase64Pain(stream);
        ...
        public void Copy()
        {
            ...
            Char[] urlChar = _encoding.GetChars(buffer).Where(n => n != 0).ToArray();
            string sBuffer = new string(urlChar);
        ...
        public void CopyEx()
        {
            if (CanCopy())
            {
                try
                {
                    ...
                    //set memorystream (BinaryData) clipboard data
                    byte[] end_buffer = _encoding.GetBytes(sBuffer);
                    System.IO.MemoryStream ms = new System.IO.MemoryStream(end_buffer, 0, end_buffer.Length, false, true);
                    da.SetData("BinaryData", ms);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (_onlyStringView)
            {
                ...
                _recStringView.X = 0;
                _recStringView.Y = 1;
                PaintString(e.Graphics, _startByte, _endByte);
            }
            else
            {
                if (_lineInfoVisible)
        ...
        void PaintLineInfo(Graphics g, long startByte, long endByte)
        {
            ...
            for (int i = 0; i < maxLine; i++)
            {
                ...
                int nulls = 10 - info.Length;
                string formattedInfo;
                if (nulls > -1)
                {
                    formattedInfo = new string('0', 10 - info.Length) + info;
                }
                else
                {
                    formattedInfo = new string('~', 10);
                }
        ...
        void PaintHexAndStringView(Graphics g, long startByte, long endByte)
        {
            ...
            if (list.Any())
            {
                ...
                /*var charBytes = new char[bytes.Length];
                int charsDecodedCount = decoder.GetChars(bytes, 0, bytes.Length, charBytes, 0);*/

                //charBytes = new char[bytes.Length];
                //if (this._encoding != null)
                //{
                // 用默认编码获取字符集
                Decoder decoder = _encoding.GetDecoder();
                char[] charBytes = GetDecodeChars(bytes, decoder);
                //}
                //else if (this._encoding == null && this._isChangeToBase64)
                //{
                //    Convert.ToBase64CharArray(bytes, 0, bytes.Length, charBytes, 0);   // Base64的方式暂时不好展示。暂时不做！
                //    this._isChangeToBase64 = false;
                //}
                //else
                //{
                //    return;
                ...
                for (long i = startByte; i < intern_endByte + 1; i++)
                {
                    ...
                    if (charBytes[counter] == 65279 || charBytes[counter] == '\0')
                    {
                        s = string.Empty;
                    }
                    else
                    {
                        s = charBytes[counter].ToString();
                    }
                    ...
                    {
                        // 改变选择区域的背景大小
                        g.FillRectangle(selBrushBack, byteStringPointF.X, byteStringPointF.Y, 2 * _charSize.Width, _charSize.Height);
        ...
        private char[] GetDecodeChars(byte[] bytes, Decoder decoder)
        {
            if (null == bytes || decoder == null)
                return null;
            ...
            while (bytes.Length > totalDecodeCount)
            {
                decodeLenth = bytes.Length - startIndex;
                charCount = decoder.GetCharCount(bytes, startIndex, decodeLenth);
                if (charCount == 0)
                    break;

                charBytes = new char[charCount];
                totalDecodeCount += decoder.GetChars(bytes, startIndex, decodeLenth, charBytes, 0);
                charBytes.CopyTo(result, startIndex);
        ...
        void UpdateRectanglePositioning()
        {
            ...
            if (_stringViewVisible)
            {
                _recStringView = new Rectangle(_recHex.X + _recHex.Width,
                    _recHex.Y + 1,
                    (int)(2 * _charSize.Width * _iHexMaxHBytes),
                    _recHex.Height);
            }
            else
            {
        ...
        {
            float x = (2 * _charSize.Width) * gp.X + _recStringView.X;
        ...
        public override Color BackColor
        {
            ...
            set
            {
                ...
                this.BackColor_Brush = new SolidBrush(value);
            }
        }

        private Brush BackColor_Brush = new SolidBrush(Color.White);
        ...
        /// <summary>
        /// Gets or sets the hex box磗 border style.
        /// </summary>
        [DefaultValue(typeof(BorderStyle), "Fixed3D"), Category("Hex"), Description("Gets or sets the hex box磗 border style.")]
        ...
        #region Set Only show String

        /// <summary>
        /// Gets or sets the visibility of the string view.
        /// </summary>
        [DefaultValue(false), Category("OnlyString"), Description("Gets or sets the OnlyStringView")]
        public bool OnlyStringView
        {
            get { return _onlyStringView; }
            set
            {
                if (_onlyStringView == value)
                    return;

                _onlyStringView = value;
                OnOnlyStringViewChanged(EventArgs.Empty);

                UpdateRectanglePositioning();
                Invalidate();
            }
        }
        bool _onlyStringView;
        #endregion

        #region Set the Only String  Event

        /// <summary>
        /// Occurs, when the value of StringViewVisible property has changed.
        /// </summary>
        [Description("Occurs, when the value of OnlyStringView property has changed.")]
        public event EventHandler OnlyStringViewChanged;
        ...
        /// </summary>
        /// <param name="e">An EventArgs that contains the event data.</param>
        protected virtual void OnOnlyStringViewChanged(EventArgs e)
        {
            if (OnlyStringViewChanged != null)
                OnlyStringViewChanged(this, e);
        }
        ...
        /// UsedFixedBytesPerLine property must set to true
        /// </remarks>
        [DefaultValue(9), Category("Hex"), Description("Gets or sets the maximum count of bytes in one line.")]
        public int FontPerLine
        {
            get { return _fontPerLine; }
            set
            {
                if (_fontPerLine == value)
                    return;
        ...
        #region Set the Font Size  Event
        ...
        #region Set String Paint
        void PaintString(Graphics g, long startByte, long endByte)
        {
            Brush brush = new SolidBrush(GetDefaultForeColor());
            Brush selBrush = new SolidBrush(_selectionForeColor);
            Brush selBrushBack = new SolidBrush(_selectionBackColor);
            ...
            if (list.Any())
            {
                ...
                decoder.GetChars(bytes, 0, bytes.Length, charBytes, 0);
                for (long i = startByte; i < intern_endByte + 1; i++)
                {
                    counter++;
                    Point gridPoint = GetGridBytePoint(counter);
                    PointF byteStringPointF = GetOnlyStringPointF(gridPoint);
                    byte b = _byteProvider.ReadByte(i);
                    ...
                    {
                        // 改变选择区域的背景大小
                        g.FillRectangle(selBrushBack, byteStringPointF.X, byteStringPointF.Y, 3 * _charSize.Width, _charSize.Height);
        ...
        #region Set String Point

        PointF GetOnlyStringPointF(Point gp)
        {
            float x = (3 * _charSize.Width) * gp.X + _recStringView.X;
            float y = (gp.Y + 1) * _charSize.Height - _charSize.Height + _recStringView.Y;
