...
namespace System.Utility
{
    ...
    internal class MacPListParse
    {
        ...
        public bool TryParseArchiver(object sourceData, out object archiverData)
        {
            ...
            if (data != null && data.ContainsKey("$archiver"))
            {
                try
                {
                    ...
                    var rootObjectIndex = topData.ContainsKey("root") ? topData["root"][0] : topData["Data"][0]; ;
            ...
            if (nsObject is IDictionary<string, object> && nsObject.ContainsKey("$class"))
            {
                ...
                switch (nsObjectBaseClass)
                {
                    ...
                    case "NSDecimalNumber":
                        ...
                        break;
                    case "NSDate":
                        nsObjectData = NsDateParse(nsObject);
                        break;
                    case "NSMutableData":
                        nsObjectData = NSMutableDataParse(nsObject);
                        break;
                    default:
                        //未解析类型
                        break;
        ...
        private object NSMutableDataParse(dynamic nsObject)
        {
            foreach (var item in nsObject)
            {
        ...
        private object NsDateParse(dynamic nsObject)
        {
            return TimeConvertHelper.NsDateToSystemTime(nsObject["NS.time"]);
        }
        ...
        {
            var baseClasses = objectsData[nsObject["$class"][0]]["$classes"];
            string nsObjectBaseClass = baseClasses[0];
            if (!IsNsBaseClass(nsObjectBaseClass))
            {
                foreach (var baseClass in baseClasses)
                {
                    if (IsNsBaseClass(baseClass))
                    {
                        nsObjectBaseClass = baseClass;
                        break;
                    }
                }
            }
        ...
        /// 基础的NS数据类型
        /// </summary>
        private readonly List<string> _nsBaseClass = new List<string> { "NSDictionary", "NSArray", "NSObject", "NSUUID", "NSURL", "NSString", "NSDate", "NSMutableData" };
        ...
        private object ParseXmlNode(XmlNode node)
        {
            switch (node.Name)
            {
                ...
                case "integer":
                    try
                    {
                        return Convert.ToInt32(node.InnerText, Globalization.NumberFormatInfo.InvariantInfo);
                    }
                    catch
                    {
                        return Convert.ToInt64(node.InnerText, Globalization.NumberFormatInfo.InvariantInfo);
                    }
                case "real":
                    return Convert.ToDouble(node.InnerText, Globalization.NumberFormatInfo.InvariantInfo);
                case "false":
                    return false;
                case "true":
                    return true;
